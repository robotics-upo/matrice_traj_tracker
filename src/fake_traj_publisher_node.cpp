...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle lnh("~");
  ros::Publisher traj_pub = lnh.advertise<trajectory_msgs::MultiDOFJointTrajectory>
    ("/input_trajectory", 1, false);
  ...
  if (argc == 3)
    {
	  ...
	  traj_point.transforms[0].rotation.w = q.getW();
        ...
        {
	  ROS_WARN("Direction of movement should be x y z or yaw");
    ...
    {
      ROS_WARN("You should give the direction of mouvement and the distance");
      ROS_WARN("Example: rosrun matrice_traj_tracker fake_traj_publisher_node");
      exit(0);
    }
  double rate;
