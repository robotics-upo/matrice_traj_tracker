...
{  
  ...
  if ( display_mode != DJISDK::DisplayMode::MODE_P_GPS ||
    ...
    {
      ROS_ERROR("\tTakeoff finished, but the aircraft is in an unexpected mode.");
      ROS_ERROR("Please connect DJI GO.");
      return false;
    }
...
void navigateGoalCallback(){
  ...
  // TODO: Okey if a new goal is while we are navigating to another goal
  // it means that the trajectory has been re-calculated. So what to do?
...
int main (int argc, char** argv)
{
  ...
  ros::Subscriber flightStatusSub = nh.subscribe("/dji_sdk/flight_status", 1,
						 &flight_status_callback);
  ros::Subscriber displayModeSub = nh.subscribe("/dji_sdk/display_mode", 1,
						&display_mode_callback);
  ros::Subscriber gps = nh.subscribe("/dji_sdk/gps_position", 1, &gps_callback);
  ros::Subscriber trajectorySub = nh.subscribe("/input_trajectory", 1,
					       &input_trajectory_callback);
  ...
  // Required services
  ctrl_authority_service = nh.serviceClient<dji_sdk::SDKControlAuthority>
  ...
  watchdogPeriod = ros::Duration(1/watchdogFreq);
